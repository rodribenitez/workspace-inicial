...
//             <button onclick = "enviarCarrito()" type ="button" class="w-20 p-3 btn-success"> Comprar </button>
//         </div>
//         <hr>
...
//     }
    ...
    function showProductsInfo(array){
        document.getElementById("container-product-info").innerHTML += `<div class = "d-flex justify-content-between">
        <h1 class"mb-3 mt-5"> `+ array.name + ` </h1>
        <button onclick = "enviarCarrito()" type ="button" class="w-20 p-3 btn-success"> Comprar </button>
    </div>
    <hr>
    ...
    `
    document.getElementById("enviarComentario").innerHTML += `<div class"mb-2 mt-2">
    <div class = "flex-column ">
        <h3 class="mt-3"> Comentar</h3>
        <p> Tu opinion:</p>
        <textarea name="nameComentario" id="textComentario" cols="30" rows="3" placeholder = "Agrega tu comentario..."></textarea>
...
<div class =" d-flex">
    ...
    document.getElementById("productoRelacionado").innerHTML = `        <div class = "d-flex w-50">
    <div onclick =setProductID(${array.relatedProducts[0].id})  class="d-flex w-100 justify-content-between m-2">
        <div class="mb-1">
            <img src="` + array.relatedProducts[0].image + `" alt="product image" class="img-thumbnail">
            <h4>`+ array.relatedProducts[0].name +`</h4> 
...
function enviarCarrito() {
    ...
    itemsProducts.count = categoriesArray.soldCount;
    itemsProducts.currency = categoriesArray.currency;
    itemsProducts.image = categoriesArray.images[0];
    itemsProducts.name = categoriesArray.name;
    itemsProducts.unitCost = categoriesArray.cost;

    
    productsCarrito.push(itemsProducts);
...
document.addEventListener("DOMContentLoaded", function(e){
    ...
    //     enviarCarrito(categoriesArray)
    // } else {
    //     productsCarrito = [];
    // }

    // localStorage.setItem("newProductCart", JSON.stringify(categoriesArray));
